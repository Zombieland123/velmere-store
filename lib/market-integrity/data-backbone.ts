...
export const tokenRiskInputSchema = z.object({
  ...
  symbol: z.string().min(1).max(32).transform((value) => value.trim().toUpperCase()),
  ...
  url: z.string().url().optional().or(z.literal("")).transform((value) => value || undefined),
...
export function validateRuntimeEnv(env: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ok: parsed.success,
    warnings: parsed.success ? [] : parsed.error.issues.map((issue) => `${issue.path.join(".")}: ${issue.message}`),
    configured: {
