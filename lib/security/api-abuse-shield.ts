...
import { applyDurableRateLimit, type DurableRateLimitDecision } from "@/lib/security/durable-rate-limit";
...
export async function applyApiAbuseShield(
  ...
  options: { queryParam?: string; keyPrefix?: string; allowEmptyQuery?: boolean } = {},
  ...
  const key = `${options.keyPrefix ?? profile}:${getClientKey(request, "client")}`;
