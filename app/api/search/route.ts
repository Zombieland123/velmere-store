import { searchVelmereIntelligence, type VelmereSearchMode, type VelmereSearchResult, buildVelmereShieldBridge } from "@/lib/search/intelligence-search-contract";
...
function compactMarketValue(value: number | null | undefined, currency = "USD") {
...
function marketResultCopy(
  ...
  const netAssets = compactMarketValue(snapshot?.fundamentals.netAssets, currency);
  ...
  if (locale === "pl") {
    return {
      ...
      next: snapshot?.state === "source_bound"
...
function marketRowToLensResult(
  ...
  const providerMissing = snapshot?.missingReason ||
    (locale === "pl"
  ...
  return {
    ...
    missingData: missingData.length ? missingData : [
      locale === "pl" ? "sprawdź najnowszy filing i drugi provider" : locale === "de" ? "neuestes Filing und Zweitprovider prüfen" : "review latest filing and second provider",
    ],
    ...
    marketSnapshot: {
      ...
      fundamentalFilingDate: sec?.filingDate || quality?.filingDate || undefined,
      fundamentalFilingUrl: sec?.filingUrl || undefined,
      fundamentalFilingForm: sec?.filingForm || quality?.filingForm || undefined,
      ...
      fundamentalRevenueTtm: quality?.revenueTtm ?? snapshot?.fundamentals.revenueTtm ?? undefined,
      ...
      fundamentalTopHoldings: snapshot?.fundamentals.topHoldings
        .slice(0, 5)
        .map((holding) => `${holding.symbol || holding.description}${holding.weight == null ? "" : ` ${holding.weight}%`}`),
      ...
      anomalyLabel: sec?.flags.slice(0, 2).join(" · ") || quality?.flags.slice(0, 2).join(" · "),
    },
  };
}
...
async function loadPass466MarketMatches(
  ...
  if (intent === "detail" && rows.length === 1 && isPass466ExactMarketMatch(rows[0], query)) {
...
function coinToLensResult(coin: CoinGeckoMarket, locale: LensLocale): VelmereSearchResult {
  const symbol = coin.symbol.toUpperCase();
  const price = typeof coin.current_price === "number" ? `$${coin.current_price.toLocaleString(undefined, { maximumFractionDigits: coin.current_price < 1 ? 6 : 2 })}` : "price pending";
  const marketCap = typeof coin.market_cap === "number" ? `$${coin.market_cap.toLocaleString(undefined, { notation: "compact", maximumFractionDigits: 2 })}` : "market-cap pending";
  const volume = typeof coin.total_volume === "number" ? `$${coin.total_volume.toLocaleString(undefined, { notation: "compact", maximumFractionDigits: 2 })}` : "volume pending";
  const fdv = typeof coin.fully_diluted_valuation === "number" ? `$${coin.fully_diluted_valuation.toLocaleString(undefined, { notation: "compact", maximumFractionDigits: 2 })}` : "FDV pending";
  const highLow = typeof coin.high_24h === "number" && typeof coin.low_24h === "number" ? `${coin.low_24h.toLocaleString(undefined, { maximumFractionDigits: coin.low_24h < 1 ? 6 : 2 })}-${coin.high_24h.toLocaleString(undefined, { maximumFractionDigits: coin.high_24h < 1 ? 6 : 2 })}` : "range pending";
  const change24hValue = coin.price_change_percentage_24h_in_currency ?? coin.price_change_percentage_24h;
  const change = typeof change24hValue === "number" ? `${change24hValue.toFixed(2)}% 24h` : "24h pending";
  const change1h = typeof coin.price_change_percentage_1h_in_currency === "number" ? `${coin.price_change_percentage_1h_in_currency.toFixed(2)}% 1h` : "1h pending";
  const change7d = typeof coin.price_change_percentage_7d_in_currency === "number" ? `${coin.price_change_percentage_7d_in_currency.toFixed(2)}% 7d` : "7d pending";
  ...
  const localized = locale === "de"
    ...
    : locale === "en"
      ...
      : {
          ...
          chips: ["rynek live", "logo z metadanych", "drugie źródło wymagane"],
        };
  return {
    ...
    summary: localized.summary,
    whyItMatters: localized.why,
    missingData: ["Binance/MEXC depth", "second-source venue agreement", "fresh source timestamp", "holder/context lane", "advanced liquidity/slippage packet"],
    nextOperatorStep: localized.next,
    ...
    bridge,
    sources: [
      { id: "coingecko-markets", label: "CoinGecko markets", mode: "live", freshness: "request-time", confidence, note: "price/market-cap/volume/FDV/high-low/logo lane" },
      { id: "venue-depth", label: "Venue depth", mode: "missing", freshness: "missing", confidence: 0, note: "requires Binance/MEXC/Coinbase/Kraken adapter" },
    ],
    ...
    marketSnapshot: {
      ...
      liquidityLabel: locale === "pl" ? "Do potwierdzenia z venue depth" : locale === "de" ? "Mit Venue Depth zu bestätigen" : "Requires venue-depth confirmation",
      depthLabel: locale === "pl" ? "Binance / MEXC / Coinbase lane wymagany" : locale === "de" ? "Binance-/MEXC-/Coinbase-Pfad erforderlich" : "Binance / MEXC / Coinbase lane required",
      holderConcentrationLabel: locale === "pl" ? "Snapshot holderów wymagany" : locale === "de" ? "Holder-Snapshot erforderlich" : "Holder snapshot required",
      unlockLabel: locale === "pl" ? "Harmonogram unlocków wymagany" : locale === "de" ? "Unlock-Zeitplan erforderlich" : "Unlock schedule required",
      venueHealthLabel: locale === "pl" ? "Status, depth i heartbeat do spięcia" : locale === "de" ? "Status, Depth und Heartbeat ausstehend" : "Status, depth and heartbeat pending",
      anomalyLabel: locale === "pl" ? "Porównaj ruch ceny z wolumenem i głębokością" : locale === "de" ? "Preisbewegung mit Volumen und Tiefe vergleichen" : "Compare price move against volume and depth",
    },
  };
}
...
async function attachPass463VenueEvidence(
  ...
  try {
    ...
    const comparisonCopy =
      locale === "pl"
        ? `PASS463: ${assetSymbol} ma parę ${primary.pair} na ${primary.venue}${secondary ? ` oraz ${secondary.pair} na ${secondary.venue}` : ""}. Stan ${comparison.state}, rozjazd ceny ${divergence}, baza ${comparison.quoteBasisState}. To kontrola jakości market data, nie certyfikat bezpieczeństwa giełdy.`
        : locale === "de"
          ? `PASS463: ${assetSymbol} nutzt ${primary.pair} auf ${primary.venue}${secondary ? ` und ${secondary.pair} auf ${secondary.venue}` : ""}. Status ${comparison.state}, Preisabweichung ${divergence}, Quotierungsbasis ${comparison.quoteBasisState}. Dies ist eine Marktdatenprüfung, kein Börsen-Sicherheitszertifikat.`
          : `PASS463: ${assetSymbol} uses ${primary.pair} on ${primary.venue}${secondary ? ` and ${secondary.pair} on ${secondary.venue}` : ""}. State ${comparison.state}, price divergence ${divergence}, quote basis ${comparison.quoteBasisState}. This is a market-data quality check, not an exchange safety certificate.`;
        ...
        ? [
            {
              ...
              freshness:
                secondary.state === "provider_error" || secondary.state === "unsupported"
                  ? "source required"
    ...
    const secondVenueGap =
      comparisonHasTwoUsableSources
        ? null
        : locale === "pl"
...
async function loadCoinGeckoMatches(query: string, locale: LensLocale) {
  ...
  try {
    ...
    const mapped = (exactOnly ? scored.slice(0, 1) : scored.slice(0, resultLimit))
...
function mergeResults(live: VelmereSearchResult[], local: VelmereSearchResult[]) {
...
function localizeMissingData(value: string, locale: LensLocale) {
  ...
  if (locale === "pl") {
    if (clean.includes("orderbook") || clean.includes("depth")) return "głębokość orderbooku dla wskazanej giełdy";
    if (clean.includes("holder")) return "świeży snapshot koncentracji holderów";
    if (clean.includes("contract")) return "weryfikacja uprawnień i kontekstu kontraktu";
    if (clean.includes("second") || clean.includes("agreement") || clean.includes("comparison")) return "potwierdzenie w drugim niezależnym źródle";
    if (clean.includes("timestamp") || clean.includes("fresh")) return "aktualny timestamp źródła";
    if (clean.includes("reserve") || clean.includes("issuer")) return "aktualny kontekst emitenta lub rezerw";
  ...
  if (clean.includes("orderbook") || clean.includes("depth")) return "Orderbuch-Tiefe für den angegebenen Handelsplatz";
  if (clean.includes("holder")) return "aktueller Snapshot der Holder-Konzentration";
  if (clean.includes("contract")) return "Prüfung von Contract-Rechten und Kontext";
  if (clean.includes("second") || clean.includes("agreement") || clean.includes("comparison")) return "Bestätigung durch eine zweite unabhängige Quelle";
  if (clean.includes("timestamp") || clean.includes("fresh")) return "aktueller Quellenzeitstempel";
  if (clean.includes("reserve") || clean.includes("issuer")) return "aktueller Emittenten- oder Reservekontext";
  if (clean.includes("wallet")) return "Implementierung und Test des Wallet-Gates";
...
function localizeResult(item: VelmereSearchResult, locale: LensLocale): VelmereSearchResult {
  if (locale === "en") return item;
  const isLiveResult = item.id.startsWith("coingecko-") || item.id.startsWith("market-");
  const symbol = item.symbol ? ` (${item.symbol})` : "";
  const sourceCount = item.sources.filter((source) => source.mode !== "missing").length;
  const localized =
    locale === "pl"
      ? {
  ...
  return {
    ...
    missingData: item.missingData.map((value) => localizeMissingData(value, locale)),
    sources: item.sources.map((source) => ({
      ...source,
      freshness:
        locale === "pl"
          ? source.freshness === "missing" ? "brak" : source.freshness === "cached" ? "cache" : source.freshness === "request-time" ? "czas zapytania" : source.freshness
          : source.freshness === "missing" ? "fehlt" : source.freshness === "cached" ? "Cache" : source.freshness === "request-time" ? "Abfragezeit" : source.freshness,
    })),
  };
}
...
export async function GET(request: Request) {
  ...
  const marketMatches =
    mode === "all" || mode === "market"
      ...
      : [];
  const liveMatches = mode === "all" || mode === "token" || mode === "contract" ? await loadCoinGeckoMatches(query, locale) : [];
  if (marketMatches.length || liveMatches.length) {
    ...
    response.productionBoundary =
      "PASS466 joins Lens to the Real Markets stock/ETF/REIT catalog. Suggest mode is catalog-only; committed exact detail searches may use Alpha Vantage and SEC/XBRL while crypto remains on CoinGecko plus venue evidence.";
  }
  response.results = response.results.map((item) => localizeResult(item, locale));
