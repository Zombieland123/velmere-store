...
// PASS327 guard compatibility: right-edge Orbit detail drawer keeps native scroll unlocked, cards return to advanced circular VLM nodes, and public trust copy is removed from customer-facing Orbit tiles.
...
import { buildShieldChatResponse, type ShieldChatSourceContext } from "@/lib/market-integrity/shield-chat";
    ...
    return (
      <div className="relative flex h-[20rem] items-center sm:h-[24rem] justify-center overflow-hidden rounded-[1.5rem] border border-white/[0.10] bg-[#070708] text-center text-sm text-white/[0.44]">
        ...
        <div className="relative max-w-sm px-6 leading-7">
          Candle feed is warming up. Shield will render fallback market
          structure when live bars are sparse.
        </div>
      </div>
    );
...
function PopupMarketChart({
  ...
  const handleChartPointerMove = (
    ...
    const pixelsPerBar = 18;
    const deltaBars =
      deltaX > 0
        ? Math.floor(deltaX / pixelsPerBar)
        : Math.ceil(deltaX / pixelsPerBar);
    if (deltaBars === 0) return;
    // PASS194 compatibility marker for preflight only: Math.round(deltaX / 10).
    // PASS412: smooth start — no one-pixel jump on first drag frame.
    // PASS269 direction is retained: the rendered window follows the hand after a small dead-zone.
    // PASS268 compatibility marker retained for static guard only: setWindowOffset(clampOffset(dragRef.current.startOffset + deltaBars));
...
function AskShieldChatPanel({
  ...
  const panelCopy = {
    pl: {
      ...
      handoffConfirmed: "Kontekst instrumentu odebrany; Shield wykonał własny świeży skan.",
      handoffBoundary: "Packet jest tylko kontekstem UI i nie zastępuje wyniku Shield AI.",
    },
    de: {
      ...
      handoffConfirmed: "Instrument-Kontext empfangen; Shield hat einen eigenen frischen Scan ausgeführt.",
      handoffBoundary: "Das Paket ist nur UI-Kontext und ersetzt keinen Shield-AI-Befund.",
    },
    en: {
      ...
      handoffConfirmed: "Instrument context received; Shield completed its own fresh scan.",
      handoffBoundary: "The packet is display-only context and never replaces the Shield AI result.",
    },
  } as const;
    ...
    fetch(
      ...
      .then((payload: Pass462ShieldVenuePayload | null) => {
        ...
        const sourceContract =
          locale === "pl"
            ? `PASS463: ${symbol} używa ${primary.venue} ${primary.pair} oraz ${secondaryLabel}. Bot może opisywać cenę, spread, świeżość, depth i bazę ${comparison.quoteBasisState}, ale nie może gwarantować wypłat, rezerw ani bezpieczeństwa giełdy.`
            : locale === "de"
              ? `PASS463: ${symbol} nutzt ${primary.venue} ${primary.pair} und ${secondaryLabel}. Der Bot darf Preis, Spread, Aktualität, Depth und die Basis ${comparison.quoteBasisState} beschreiben, aber keine Auszahlungen, Reserven oder Börsensicherheit garantieren.`
              : `PASS463: ${symbol} uses ${primary.venue} ${primary.pair} and ${secondaryLabel}. The bot may describe price, spread, freshness, depth and the ${comparison.quoteBasisState} basis, but cannot guarantee withdrawals, reserves or venue safety.`;
        const providerPlan =
        ...
        const providerFacts = [
          {
            label: locale === "pl" ? "Venue główne" : locale === "de" ? "Primärer Handelsplatz" : "Primary venue",
            value: `${primary.assetSymbol} · ${primary.venue} ${primary.pair} · ${primary.state} · ${primary.healthScore}/100`,
          },
          {
            label: locale === "pl" ? "Pokrycie pary" : locale === "de" ? "Paarabdeckung" : "Pair coverage",
            value: `${primary.pairResolutionState} · ${primary.quoteCurrency}`,
          },
          ...(secondary
            ? [
                {
                  label: locale === "pl" ? "Drugie venue" : locale === "de" ? "Zweiter Handelsplatz" : "Secondary venue",
                  value: `${secondary.venue} ${secondary.pair} · ${secondary.state} · ${secondary.healthScore}/100`,
                },
              ]
            : []),
          {
            label: locale === "pl" ? "Baza kwotowania" : locale === "de" ? "Quotierungsbasis" : "Quote basis",
          ...
          },
          {
            ...
            value:
              comparison.priceDivergenceBps == null
                ? locale === "pl" ? "wymagane porównywalne źródło" : locale === "de" ? "vergleichbare Quelle erforderlich" : "comparable source required"
                : `${comparison.priceDivergenceBps.toFixed(1)} bps`,
          },
          {
            label: locale === "pl" ? "Spread / świeżość" : locale === "de" ? "Spread / Aktualität" : "Spread / freshness",
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span className="rounded-full border border-cyan-200/[0.12] bg-black/[0.18] px-2 py-1 font-mono text-[7px] uppercase tracking-[0.11em] text-cyan-50/[0.58]">
              {handoffContext.depth.toUpperCase()} · {handoffContext.sourceConfidence}% · {handoffContext.sourceMode}
            </span>
          </div>
      ...
      <div className="flex min-w-0 flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          ...
          <p className="shield-copy-safe mt-1 text-xs leading-6 text-white/[0.48]">
            PASS77 opens with a concise review deck first: operator brief,
            source truth, AI boundaries, evidence gate, interaction path and
            launch blockers before deep panels.
          </p>
        </div>
    ...
    <div className="shield-terminal-loader mt-4 p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        {Array.from({ length: 3 }).map((_, index) => (
          <div
            key={index}
            className="h-20 animate-pulse rounded-2xl border border-white/[0.07] bg-white/[0.028]"
          />
        ))}
      </div>
    </div>
...
function VlmAiSequenceOverlay({
  ...
  const readNodes = useMemo<VlmReadNode[]>(() => {
    ...
    const advancedReadNodes: VlmReadNode[] = [
      ...
      },
      {
        ...
        hint: "operator read",
        detail:
          "Final operator readout explains the next safest check: verify sources, inspect liquidity, review holders, audit contract or draft evidence.",
        x: 66,
        y: 92,
        tone: riskScore >= 55 ? "red" : "green",
        group: "signals",
          ...
          result.dataQuality === "live" && severityTone !== "blocked"
            ...
            : isPolish
              ...
              : isGerman
                ...
                : "hold as internal",
        detail: isPolish
          ? "PASS208 łączy mózg AI z bezpiecznym raportem, ale nie generuje jeszcze binarnego PDF."
          : isGerman
            ? "PASS208 verbindet AI Brain mit sicherem Report, erzeugt aber noch kein binäres PDF."
            : "PASS208 bridges AI Brain to safe reporting, but does not create the binary PDF yet.",
      },
    ];
  ...
  // PASS324 compatibility marker: onWheel={(event) => event.stopPropagation()} upgraded by PASS325 to forced wheel-capture scroll.
            ...
            >
              ...
              >
                ...
                {selectedTileMegaBranchControlTower ? (
                  ...
                  >
                    <div className="shield-vlm-release-chain-head">
                      <strong>PASS233–242 control tower</strong>
                      <span>
                ...
                {selectedTileReleaseTriageBoard ? (
                  ...
                  >
                    <div className="shield-vlm-release-chain-head">
                      <strong>PASS243 release triage</strong>
                      <span>
                ...
                {selectedTileOperatorHandoffVault ? (
                  ...
                  >
                    <div className="shield-vlm-release-chain-head">
                      <strong>PASS244 handoff vault</strong>
                      <span>
                ...
                {selectedTileBrowserReplayScript ? (
                  ...
                  >
                    <div className="shield-vlm-release-chain-head">
                      <strong>PASS245 browser replay</strong>
                      <span>
                ...
                {selectedTileReleaseReadinessOrchestrator ? (
                  ...
                  >
                    <div className="shield-vlm-release-chain-head">
                      <strong>PASS251 release readiness orchestrator</strong>
                      <span>
                ...
                <div className="shield-vlm-detail-operator-footrail mt-3 grid gap-2 rounded-2xl border border-white/[0.10] bg-black/[0.50] p-3 font-mono text-[9px] uppercase tracking-[0.12em] text-white/[0.50] sm:grid-cols-3">
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div
            className="shield-vlm-motion-toggle-mini shield-vlm-orbit-only"
            aria-label={ui.motionGovernor}
          >
            <button
              type="button"
              className="is-active"
          ...
          {showMotionQaHud && motionPreset !== "static" ? (
            <div
              className="shield-vlm-zoom-controls"
              aria-label="Orbit zoom QA"
              data-vlm-no-drag="true"
              data-vlm-motion-qa="true"
            >
              <button
                ...
                onClick={() =>
                  setOrbitZoom((current) =>
                    Number(Math.max(0.86, current - 0.06).toFixed(3)),
                  )
                }
              >
                −
              ...
              <button
                ...
                onClick={() =>
                  setOrbitZoom((current) =>
                    Number(Math.min(1.16, current + 0.06).toFixed(3)),
                  )
                }
              >
                +
      ...
      <div
        className="shield-vlm-pass368-brain-timeline"
        data-pass368-brain-build-timer="true"
        data-vlm-no-drag="true"
        onPointerDown={(event) => event.stopPropagation()}
      >
        <span>{phaseLabel}</span>
        <strong>{estimatedReadSeconds}s</strong>
        <em>
          {readNodes.length} evidence nodes · source → liquidity → holders →
          output
        </em>
      </div>
      ...
      <section
        ...
        data-vlm-no-drag="true"
        onPointerDown={(event) => event.stopPropagation()}
        aria-label="PASS395 VLM neural collection before audit readout"
      >
...
export default function TokenRiskModal({
  ...
  const ui = useMemo(() => {
    if (locale === "pl") {
      return {
        ...
        evidenceExportBody:
          "Manifest pokazuje źródła, braki danych i checklistę operatora. To nadal preview, nie finalny raport prawny.",
        ...
        infoLabel: "opis",
        tileDetails: "szczegóły",
        caseKicker: "sprawa operatora",
        caseTitle: "Kontekst do dalszej weryfikacji",
        caseNext: "następny krok",
        caseBlockers: "blokery",
        caseQueries: "zapytanie OSINT",
        summaryDisclaimer:
          "VLM porządkuje sygnały, źródła i braki danych w czytelny operator brief.",
      };
    }
    if (locale === "de") {
      return {
        ...
        evidenceExportBody:
          "Das Manifest zeigt Quellen, Datenlücken und Operator-Checkliste. Es bleibt ein Preview, kein finaler Rechtsbericht.",
        ...
        infoLabel: "Info",
        tileDetails: "Details",
        caseKicker: "Operator-Fall",
        caseTitle: "Kontext für weitere Prüfung",
        caseNext: "nächster Schritt",
        caseBlockers: "Blocker",
        caseQueries: "OSINT-Abfrage",
        summaryDisclaimer:
          "VLM ordnet Signale, Quellen und Datenlücken in einem klaren Operator Brief.",
      };
    }
    return {
      controlKicker: "analysis control",
      ...
      evidenceExportTitle: "Evidence manifest",
      evidenceExportBody:
        "The manifest shows sources, missing data and the operator checklist. It remains a preview, not a final legal report.",
  ...
  const layoutSentinelMode: VlmAiSequenceMode = vlmSequenceMode ?? "basic";
  ...
  const sourceSpineRows = [
      ...
      tone: orderbookLoading
        ...
        : orderbook
          ...
          : orderbookError
            ? "blocked"
            : "missing",
      state: orderbookLoading
        ? sourceSpineCopy.loading
        : orderbook
          ? sourceSpineCopy.live
          : orderbookError
            ? sourceSpineCopy.blocked
            : sourceSpineCopy.missing,
      detail: orderbook
    ...
    },
    {
      ...
      tone:
        result.metrics.holderCount || result.metrics.top10HolderPercent
          ? "partial"
          : "missing",
      state:
        result.metrics.holderCount || result.metrics.top10HolderPercent
  ...
  const sourceSpineReadyCount = sourceSpineRows.filter(
    (row) => row.tone === "live" || row.tone === "partial",
  ).length;
  ...
  const auditCopy = {
    ...
    de: {
      ...
      missing: "Ein fehlender Wert bleibt sichtbar und benötigt eine Quelle.",
    },
    en: {
      price: "Price",
      change1h: "1h change",
  ...
  const vlmAuditEvidence = useMemo(() => {
    ...
    const metric = (
      ...
      status: "verified" | "review" | "missing" = "verified",
    ) => ({
      id,
      label,
      value,
    ...
    const supplyRatio =
      result.metrics.circulatingSupply !== undefined && supplyBase
        ? `${((result.metrics.circulatingSupply / supplyBase) * 100).toFixed(1)}%`
        : null;
    const tax =
      result.metrics.buyTaxPercentage !== undefined ||
      result.metrics.sellTaxPercentage !== undefined
        ? `${formatPercent(result.metrics.buyTaxPercentage)} / ${formatPercent(result.metrics.sellTaxPercentage)}`
        : null;
    const depth = orderbook
      ? `${formatUsd(orderbook.bidDepthUsd)} / ${formatUsd(orderbook.askDepthUsd)}`
      : null;
    const turnover =
      result.metrics.volume24h !== undefined && result.metrics.marketCap
        ? (result.metrics.volume24h / result.metrics.marketCap) * 100
        : null;
    const liquidityRatio =
      result.metrics.liquidityUsd !== undefined && result.metrics.marketCap
        ? (result.metrics.liquidityUsd / result.metrics.marketCap) * 100
        : null;
    const unlockPressure =
      result.metrics.circulatingSupply !== undefined && supplyBase
        ? Math.max(
            0,
            100 - (result.metrics.circulatingSupply / supplyBase) * 100,
          )
        : null;
    const agentScores = (
      result.agentAssessments ||
    ...
    const agentDisagreement =
      agentScores.length > 1
        ? Math.max(...agentScores) - Math.min(...agentScores)
        : null;
    const sourceQuorum = result.dataSources.filter(Boolean).length;
    return buildUnifiedAuditEvidence(
      {
        locale: auditLocale,
        subject: `${asset.symbol} · ${asset.name}`,
        source: result.dataSources.join(" + ") || chartSource,
        sourceTimestamp: Number.isFinite(Date.parse(result.generatedAt))
          ? Math.floor(Date.parse(result.generatedAt) / 1000)
          : null,
        riskScore: combinedScore,
        confidence:
          result.confidence ?? result.metaModel?.dataFusionScore ?? null,
        metrics: [
          metric(
            "price",
            auditCopy.price,
          ...
          metric(
            "change30d",
            auditCopy.change30d,
            result.metrics.priceChange30d !== undefined
              ? formatPercent(result.metrics.priceChange30d)
              : null,
          ),
          metric(
            "marketCap",
          ...
          metric(
            ...
            auditLocale === "pl"
              ...
              : auditLocale === "de"
                ? "Volumen / Market Cap"
                : "Volume / market cap",
            turnover !== null ? formatPercent(turnover) : null,
            "review",
          ),
          metric(
            "liquidity",
          ...
          metric(
            "liquidityRatio",
            auditLocale === "pl"
              ? "Płynność / kapitalizacja"
              : auditLocale === "de"
                ? "Liquidität / Market Cap"
                : "Liquidity / market cap",
            liquidityRatio !== null ? formatPercent(liquidityRatio) : null,
            "review",
          ),
          metric(
            "fdv",
          ...
          metric("supply", auditCopy.supply, supplyRatio, "review"),
          metric(
            "unlockPressure",
            auditLocale === "pl"
              ? "Potencjalna presja podaży"
              : auditLocale === "de"
                ? "Potentieller Angebotsdruck"
                : "Potential supply pressure",
            unlockPressure !== null ? formatPercent(unlockPressure) : null,
            "review",
          ),
          metric(
            "holders",
          ...
          metric("depth", auditCopy.depth, depth, "review"),
          metric("candles", auditCopy.candles, candles.length || null),
          metric(
            "data",
            auditCopy.data,
            result.dataQuality,
            result.dataQuality === "live" ? "verified" : "review",
          ),
          metric("signals", auditCopy.signals, result.signals.length, "review"),
          metric(
            "agents",
            auditCopy.agents,
          ...
          metric(
            ...
            agentDisagreement !== null
              ? `${agentDisagreement.toFixed(1)} pkt`
              : null,
            "review",
          ),
          metric(
            ...
            auditLocale === "pl"
              ...
              : auditLocale === "de"
                ? "Quellenquorum"
                : "Source quorum",
            sourceQuorum ? `${sourceQuorum} / 3` : null,
            sourceQuorum >= 2 ? "verified" : "review",
          ),
          metric(
            "gaps",
            auditCopy.gaps,
            result.metaModel?.limitations?.length ?? 0,
            (result.metaModel?.limitations?.length ?? 0) > 0
              ? "review"
              : "verified",
          ),
        ],
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-1 flex min-w-0 flex-wrap items-end gap-2">
                  ...
                  <p className="max-w-[12rem] truncate pb-1 text-sm text-white/[0.48]">
                    {asset.name}
                  </p>
                  {asset.rank ? (
                    <span className="mb-1 rounded-full border border-white/[0.10] px-2 py-1 font-mono text-[9px] text-white/[0.38]">
                      #{asset.rank}
                    </span>
                  ) : null}
                </div>
            ...
            <main className="min-w-0">
              <div className="shield-token-chart-card">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div className="min-w-0">
                    <p className="font-mono text-[10px] uppercase tracking-[0.17em] text-white/[0.34]">
                      price
                    </p>
                    <div className="mt-1 flex flex-wrap items-end gap-3">
                      ...
                      <span
                        className={`rounded-full border px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.14em] ${Number(result.metrics.priceChange24h ?? 0) >= 0 ? "border-emerald-300/[0.18] bg-emerald-400/[0.07] text-emerald-100" : "border-red-300/[0.18] bg-red-400/[0.07] text-red-100"}`}
                      >
                  ...
                  <div className="flex min-w-0 flex-wrap gap-2 md:justify-end">
                    {(["1h", "4h", "1d", "7d"] as MarketChartRange[]).map(
                      (item) => (
                        <button
                          key={item}
                          type="button"
                          ...
                          data-testid={`shield-chart-range-${item}`}
                ...
                <div className="mt-4 flex flex-col gap-2 border-t border-white/[0.07] pt-4 font-mono text-[10px] uppercase tracking-[0.13em] text-white/[0.30] sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <span>{safeChartStatusLabel}</span>
                </div>
              </div>
            </main>

            <aside
              className="shield-token-action-panel"
              data-handoff-context={handoffContext ? "present" : "none"}
              data-pass269-compact-mode-dock
              data-pass342-token-side-clean="true"
              data-pass344-token-public-digest="true"
            >
              <div className="shield-pass269-action-header">
                <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-velmere-gold">
                  {ui.controlKicker}
                </p>
                <div>
                  <span>{assetRegime.displayName}</span>
                  <strong>{assetRegime.proofBadge}</strong>
                </div>
              </div>

              <div className="mt-4 grid gap-3">
                <div className="shield-analysis-row shield-analysis-row-clean">
                  <button
                    type="button"
              ...
              <details
                className="group mt-4 rounded-[1.35rem] border border-white/[0.08] bg-black/[0.16] p-3"
                data-pass457-progressive-operator-diagnostics="true"
              >
                <summary className="flex cursor-pointer list-none items-center justify-between gap-3 rounded-2xl px-2 py-1 font-mono text-[9px] uppercase tracking-[0.14em] text-white/[0.46] marker:content-none">
                  <span>
                    {auditLocale === "pl"
                  ...
                  <span className="rounded-full border border-white/[0.08] px-2 py-1 text-[8px] text-white/[0.30] group-open:text-velmere-gold">
                    {auditLocale === "pl"
                      ? "rozwiń"
                      : auditLocale === "de"
                        ? "öffnen"
                ...
                <div className="mt-3 grid gap-3">
                  <div
                    className="shield-pass268-trust-rail"
                    data-pass268-trust-rail
                  >
                    <span>chart-first</span>
                    <span>status lane</span>
                    <span>MwSt-safe copy</span>
                  </div>

                  <div
                    className="shield-pass269-regime-strip"
                    data-pass269-asset-regime-gate
                  >
                    {assetRegime.rails.map((rail) => (
                      <button
                        key={rail.id}
                        type="button"
                        className={`shield-pass269-regime-chip shield-pass269-regime-chip-${rail.tone}`}
                        data-pass342-regime-popup-ready="true"
                        title={`${rail.label}: ${rail.value} · ${assetRegime.reviewFocus}`}
                      >
                        <span>{rail.label}</span>
                        <strong>{rail.value}</strong>
                        <em className="shield-pass342-regime-mini-popover">
                          {rail.label}: {rail.value} · {assetRegime.reviewFocus}
                        </em>
                      </button>
                    ))}
                  </div>
                  ...
                  <div
                    className={`shield-pass270-pressure-rail shield-pass270-pressure-rail-${marketPressureRegime.status}`}
                    data-pass270-market-pressure-rail
                  >
                    <div className="shield-pass270-pressure-head">
                      <span>{marketPressureRegime.headline}</span>
                      <strong>{marketPressureRegime.trustBadge}</strong>
                    </div>
                    <p>{marketPressureRegime.operatorCue}</p>
                    <div className="shield-pass270-pressure-grid">
                      {marketPressureRegime.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass270-pressure-chip shield-pass270-pressure-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                  ...
                  <div
                    className={`shield-pass271-contract-trap shield-pass271-contract-trap-${contractTrapRegime.status}`}
                    data-pass271-contract-trap-gate
                  >
                    <div className="shield-pass271-contract-head">
                      <span>{contractTrapRegime.headline}</span>
                      <strong>{contractTrapRegime.trustBadge}</strong>
                    </div>
                    <p>{contractTrapRegime.operatorCue}</p>
                    <div className="shield-pass271-contract-grid">
                      {contractTrapRegime.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass271-contract-chip shield-pass271-contract-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div className="shield-pass271-contract-next">
                      <span>next</span>
                      <strong>{contractTrapRegime.nextAction}</strong>
                    </div>
                  </div>

                  <div
                    className={`shield-pass274-unlock-gate shield-pass274-unlock-gate-${unlockVestingGate.status}`}
                    data-pass274-unlock-vesting-gate
                  >
                    <div className="shield-pass274-unlock-head">
                      <span>{unlockVestingGate.headline}</span>
                      <strong>{unlockVestingGate.trustBadge}</strong>
                    </div>
                    <p>{unlockVestingGate.operatorCue}</p>
                    <div className="shield-pass274-unlock-grid">
                      {unlockVestingGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass274-unlock-chip shield-pass274-unlock-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div className="shield-pass274-unlock-next">
                      <span>next</span>
                      <strong>{unlockVestingGate.nextAction}</strong>
                    </div>
                  </div>

                  <div
                    className={`shield-pass275-osint-gate shield-pass275-osint-gate-${osintNarrativeGate.status}`}
                    data-pass275-osint-narrative-gate
                  >
                    <div className="shield-pass275-osint-head">
                      <span>{osintNarrativeGate.headline}</span>
                      <strong>{osintNarrativeGate.trustBadge}</strong>
                    </div>
                    <p>{osintNarrativeGate.operatorCue}</p>
                    <div className="shield-pass275-osint-grid">
                      {osintNarrativeGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass275-osint-chip shield-pass275-osint-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div className="shield-pass275-osint-next">
                      <span>next</span>
                      <strong>{osintNarrativeGate.nextAction}</strong>
                    </div>
                  </div>

                  <div
                    className={`shield-pass276-source-quorum-gate shield-pass276-source-quorum-gate-${sourceAdapterQuorumGate.status}`}
                    data-pass276-source-adapter-quorum-gate
                  >
                    <div className="shield-pass276-source-quorum-head">
                      <span>{sourceAdapterQuorumGate.headline}</span>
                      <strong>{sourceAdapterQuorumGate.trustBadge}</strong>
                    </div>
                    <p>{sourceAdapterQuorumGate.operatorCue}</p>
                    <div className="shield-pass276-source-quorum-grid">
                      {sourceAdapterQuorumGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass276-source-quorum-chip shield-pass276-source-quorum-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div className="shield-pass276-source-quorum-next">
                      <span>next</span>
                      <strong>{sourceAdapterQuorumGate.nextAction}</strong>
                    </div>
                  </div>

                  <div
                    className={`shield-pass279-freshness-registry shield-pass279-freshness-registry-${sourceFreshnessRegistryGate.status}`}
                    data-pass279-source-freshness-registry-gate
                  >
                    <div className="shield-pass279-freshness-registry-head">
                      <span>{sourceFreshnessRegistryGate.headline}</span>
                      <strong>{sourceFreshnessRegistryGate.trustBadge}</strong>
                    </div>
                    <p>{sourceFreshnessRegistryGate.operatorCue}</p>
                    <div className="shield-pass279-freshness-registry-grid">
                      {sourceFreshnessRegistryGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass279-freshness-registry-chip shield-pass279-freshness-registry-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass279-freshness-registry-entries"
                      aria-label="source freshness registry lanes"
                    >
                      {sourceFreshnessRegistryGate.entries.map((entry) => (
                        <span
                          key={entry.id}
                          data-pass279-freshness-entry={entry.state}
                        >
                          <strong>{entry.label}</strong>
                          <b>{entry.state.replaceAll("_", " ")}</b>
                          <small>{entry.note}</small>
                        </span>
                      ))}
                    </div>
                    <div className="shield-pass279-freshness-registry-next">
                      <span>next</span>
                      <strong>{sourceFreshnessRegistryGate.nextAction}</strong>
                      <small>
                        {sourceFreshnessRegistryGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass280-analytics-taxonomy shield-pass280-analytics-taxonomy-${analyticsEventTaxonomyGate.status}`}
                    data-pass280-analytics-event-taxonomy-gate
                  >
                    <div className="shield-pass280-analytics-taxonomy-head">
                      <span>{analyticsEventTaxonomyGate.headline}</span>
                      <strong>{analyticsEventTaxonomyGate.trustBadge}</strong>
                    </div>
                    <p>{analyticsEventTaxonomyGate.operatorCue}</p>
                    <div className="shield-pass280-analytics-taxonomy-grid">
                      {analyticsEventTaxonomyGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass280-analytics-taxonomy-chip shield-pass280-analytics-taxonomy-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass280-analytics-taxonomy-lanes"
                      aria-label="analytics event taxonomy lanes"
                    >
                      {analyticsEventTaxonomyGate.lanes.map((lane) => (
                        <span
                          key={lane.id}
                          data-pass280-analytics-event={lane.status}
                        >
                          <strong>{lane.label}</strong>
                          <b>
                            {lane.intent.replaceAll("_", " ")} ·{" "}
                            {lane.privacy.replaceAll("_", " ")}
                          </b>
                          <small>{lane.note}</small>
                        </span>
                      ))}
                    </div>
                    <div className="shield-pass280-analytics-taxonomy-next">
                      <span>next</span>
                      <strong>{analyticsEventTaxonomyGate.nextAction}</strong>
                      <small>
                        {analyticsEventTaxonomyGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass282-privacy-redaction-envelope shield-pass282-privacy-redaction-envelope-${privacyRedactionEnvelopeGate.status}`}
                    data-pass282-privacy-redaction-envelope-gate
                  >
                    <div className="shield-pass282-privacy-redaction-envelope-head">
                      <span>{privacyRedactionEnvelopeGate.headline}</span>
                      <strong>{privacyRedactionEnvelopeGate.trustBadge}</strong>
                    </div>
                    <p>{privacyRedactionEnvelopeGate.operatorCue}</p>
                    <div className="shield-pass282-privacy-redaction-envelope-grid">
                      {privacyRedactionEnvelopeGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass282-privacy-redaction-envelope-chip shield-pass282-privacy-redaction-envelope-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass282-privacy-redaction-envelope-lanes"
                      aria-label="privacy redaction envelope lanes"
                    >
                      {privacyRedactionEnvelopeGate.lanes.map((lane) => (
                        <span
                          key={lane.id}
                          data-pass282-redaction-lane={lane.exposure}
                        >
                          <strong>{lane.label}</strong>
                          <b>
                            {lane.exposure.replaceAll("_", " ")} ·{" "}
                            {lane.maskRule}
                          </b>
                          <small>{lane.note}</small>
                        </span>
                    ...
                    <div className="shield-pass282-privacy-redaction-envelope-next">
                      <span>next</span>
                      <strong>{privacyRedactionEnvelopeGate.nextAction}</strong>
                      <small>
                        {privacyRedactionEnvelopeGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass283-operator-case-sla shield-pass283-operator-case-sla-${operatorCaseSlaOrchestratorGate.status}`}
                    data-pass283-operator-case-sla-orchestrator-gate
                  >
                    <div className="shield-pass283-operator-case-sla-head">
                      <span>{operatorCaseSlaOrchestratorGate.headline}</span>
                      <strong>
                        {operatorCaseSlaOrchestratorGate.trustBadge}
                      </strong>
                    </div>
                    <p>{operatorCaseSlaOrchestratorGate.operatorCue}</p>
                    <div className="shield-pass283-operator-case-sla-grid">
                      {operatorCaseSlaOrchestratorGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass283-operator-case-sla-chip shield-pass283-operator-case-sla-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass283-operator-case-sla-stages"
                      aria-label="operator case SLA stages"
                    >
                      {operatorCaseSlaOrchestratorGate.stages.map((stage) => (
                        <span
                          key={stage.id}
                          data-pass283-case-stage={stage.state}
                        >
                          <strong>{stage.label}</strong>
                          <b>
                            {stage.priority} ·{" "}
                            {stage.state.replaceAll("_", " ")} · {stage.sla}
                          </b>
                          <small>{stage.note}</small>
                        </span>
                      ))}
                    </div>
                    <div className="shield-pass283-operator-case-sla-next">
                      <span>next</span>
                      <strong>
                        {operatorCaseSlaOrchestratorGate.nextAction}
                      </strong>
                      <small>
                        {operatorCaseSlaOrchestratorGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass281-storage-adapter-contract shield-pass281-storage-adapter-contract-${storageAdapterContractGate.status}`}
                    data-pass281-storage-adapter-contract-gate
                  >
                    <div className="shield-pass281-storage-adapter-contract-head">
                      <span>{storageAdapterContractGate.headline}</span>
                      <strong>{storageAdapterContractGate.trustBadge}</strong>
                    </div>
                    <p>{storageAdapterContractGate.operatorCue}</p>
                    <div className="shield-pass281-storage-adapter-contract-grid">
                      {storageAdapterContractGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass281-storage-adapter-contract-chip shield-pass281-storage-adapter-contract-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass281-storage-adapter-contract-lanes"
                      aria-label="storage adapter contract lanes"
                    >
                      {storageAdapterContractGate.lanes.map((lane) => (
                        <span
                          key={lane.id}
                          data-pass281-storage-lane={lane.state}
                        >
                          <strong>{lane.label}</strong>
                          <b>
                            {lane.writeTarget.replaceAll("_", " ")} ·{" "}
                    ...
                    <div className="shield-pass281-storage-adapter-contract-next">
                      <span>next</span>
                      <strong>{storageAdapterContractGate.nextAction}</strong>
                      <small>
                        {storageAdapterContractGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass284-retention-policy shield-pass284-retention-policy-${retentionPolicyGate.status}`}
                    data-pass284-retention-policy-gate
                  >
                    <div className="shield-pass284-retention-policy-head">
                      <span>{retentionPolicyGate.headline}</span>
                      <strong>{retentionPolicyGate.trustBadge}</strong>
                    </div>
                    <p>{retentionPolicyGate.operatorCue}</p>
                    <div className="shield-pass284-retention-policy-grid">
                      {retentionPolicyGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass284-retention-policy-chip shield-pass284-retention-policy-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass284-retention-policy-lanes"
                      aria-label="retention policy lanes"
                    >
                      {retentionPolicyGate.lanes.map((lane) => (
                        <span
                          key={lane.id}
                          data-pass284-retention-lane={lane.state}
                        >
                          <strong>{lane.label}</strong>
                          <b>
                            {lane.ttl} ·{" "}
                            {lane.evidenceClass.replaceAll("_", " ")}
                          </b>
                          <small>{lane.note}</small>
                        </span>
                      ))}
                    </div>
                    <div className="shield-pass284-retention-policy-next">
                      <span>next</span>
                      <strong>{retentionPolicyGate.nextAction}</strong>
                      <small>{retentionPolicyGate.customerBoundary}</small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass285-customer-risk-brief shield-pass285-customer-risk-brief-${customerSafeRiskBriefGate.status}`}
                    data-pass285-customer-safe-risk-brief-gate
                  >
                    <div className="shield-pass285-customer-risk-brief-head">
                      <span>{customerSafeRiskBriefGate.headline}</span>
                      <strong>{customerSafeRiskBriefGate.trustBadge}</strong>
                    </div>
                    <p>{customerSafeRiskBriefGate.operatorCue}</p>
                    <div className="shield-pass285-customer-risk-brief-grid">
                      {customerSafeRiskBriefGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass285-customer-risk-brief-chip shield-pass285-customer-risk-brief-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass285-customer-risk-brief-lanes"
                      aria-label="customer safe risk brief lanes"
                    >
                      {customerSafeRiskBriefGate.lanes.map((lane) => (
                        <span
                          key={lane.id}
                          data-pass285-customer-brief-lane={lane.state}
                        >
                          <strong>{lane.label}</strong>
                          <b>
                            {lane.value} · {lane.copyClass.replaceAll("_", " ")}
                          </b>
                          <small>{lane.note}</small>
                        </span>
                      ))}
                    ...
                    <div
                      className="shield-pass285-customer-risk-brief-copy"
                      aria-label="customer safe brief preview"
                    >
                      {customerSafeRiskBriefGate.customerBrief.map((line) => (
                        <span key={line}>{line}</span>
                      ))}
                    </div>
                    <div className="shield-pass285-customer-risk-brief-next">
                      <span>next</span>
                      <strong>{customerSafeRiskBriefGate.nextAction}</strong>
                      <small>
                        {customerSafeRiskBriefGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass286-lens-report-preview shield-pass286-lens-report-preview-${lensReportPreviewGate.status}`}
                    data-pass286-lens-report-preview-gate
                  >
                    <div className="shield-pass286-lens-report-preview-head">
                      <span>{lensReportPreviewGate.headline}</span>
                      <strong>{lensReportPreviewGate.trustBadge}</strong>
                    </div>
                    <p>{lensReportPreviewGate.operatorCue}</p>
                    <div className="shield-pass286-lens-report-preview-grid">
                      {lensReportPreviewGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass286-lens-report-preview-chip shield-pass286-lens-report-preview-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass286-lens-report-preview-scroll"
                      aria-label="Lens report Proof Passport Scroll sections"
                    >
                      {lensReportPreviewGate.sections.map((section) => (
                        <span
                          key={section.id}
                          data-pass286-report-section={section.state}
                        >
                          <strong>{section.label}</strong>
                          <b>
                            {section.value} · {section.layout}
                          </b>
                    ...
                    <div
                      className="shield-pass286-lens-report-preview-copy"
                      aria-label="Lens report preview copy"
                    >
                      {lensReportPreviewGate.previewLines.map((line) => (
                        <span key={line}>{line}</span>
                      ))}
                    </div>
                    <div className="shield-pass286-lens-report-preview-next">
                      <span>next</span>
                      <strong>{lensReportPreviewGate.nextAction}</strong>
                      <small>{lensReportPreviewGate.customerBoundary}</small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass287-evidence-note-integrity shield-pass287-evidence-note-integrity-${evidenceNoteIntegrityGate.status}`}
                    data-pass287-evidence-note-integrity-gate
                  >
                    <div className="shield-pass287-evidence-note-integrity-head">
                      <span>{evidenceNoteIntegrityGate.headline}</span>
                      <strong>{evidenceNoteIntegrityGate.trustBadge}</strong>
                    </div>
                    <p>{evidenceNoteIntegrityGate.operatorCue}</p>
                    <div className="shield-pass287-evidence-note-integrity-grid">
                      {evidenceNoteIntegrityGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass287-evidence-note-integrity-chip shield-pass287-evidence-note-integrity-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass287-evidence-note-integrity-lines"
                      aria-label="Evidence Note Mirror lines"
                    >
                      {evidenceNoteIntegrityGate.lines.map((line) => (
                        <span
                          key={line.id}
                          data-pass287-evidence-note-line={line.state}
                        >
                          <strong>{line.label}</strong>
                          <b>{line.value}</b>
                    ...
                    <div
                      className="shield-pass287-evidence-note-integrity-preview"
                      aria-label="Evidence Note Mirror preview"
                    >
                      {evidenceNoteIntegrityGate.previewNote.map((line) => (
                        <span key={line}>{line}</span>
                      ))}
                    </div>
                    <div className="shield-pass287-evidence-note-integrity-next">
                      <span>next</span>
                      <strong>{evidenceNoteIntegrityGate.nextAction}</strong>
                      <small>
                        {evidenceNoteIntegrityGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass288-pdf-forge shield-pass288-pdf-forge-${pdfForgeComposerGate.status}`}
                    data-pass288-pdf-forge-composer-gate
                  >
                    <div className="shield-pass288-pdf-forge-head">
                      <span>{pdfForgeComposerGate.headline}</span>
                      <strong>{pdfForgeComposerGate.trustBadge}</strong>
                    </div>
                    <p>{pdfForgeComposerGate.operatorCue}</p>
                    <div
                      className="shield-pass288-pdf-forge-animation"
                      aria-label="VLM PDF creation animation"
                    >
                      {pdfForgeComposerGate.stages.map((stage, index) => (
                        <span
                          key={stage.id}
                          data-pass288-pdf-stage={stage.state}
                          style={{ "--forge-index": index } as CSSProperties}
                        >
                          <i aria-hidden="true" />
                          <strong>{stage.label}</strong>
                          <b>{stage.value}</b>
                    ...
                    <div
                      className="shield-pass288-pdf-forge-signature"
                      aria-label="Velmère Cybersecurity signature preview"
                    >
                      {pdfForgeComposerGate.signatureLines.map((line) => (
                        <span key={line}>{line}</span>
                      ))}
                    </div>
                    <div className="shield-pass288-pdf-forge-actions">
                      <button
                        type="button"
                        onClick={downloadVelmereCybersecurityPdf}
                      >
                        {pdfForgeComposerGate.downloadLabel}
                      </button>
                      <span>
                        {pdfForgeNotice === "downloaded"
                          ? "PDF ready"
                          : pdfForgeNotice === "failed"
                            ? "PDF failed"
                            : pdfForgeComposerGate.customerBoundary}
                      </span>
                    </div>
                  </div>

                  <div
                    className={`shield-pass289-layout-sentinel shield-pass289-layout-sentinel-${layoutStabilitySentinelGate.status}`}
                    data-pass289-layout-stability-sentinel
                  >
                    <div className="shield-pass289-layout-sentinel-head">
                      <span>{layoutStabilitySentinelGate.headline}</span>
                      <strong>{layoutStabilitySentinelGate.trustBadge}</strong>
                    </div>
                    <p>{layoutStabilitySentinelGate.operatorCue}</p>
                    <div className="shield-pass289-layout-rail-grid">
                      {layoutStabilitySentinelGate.rails.map((rail) => (
                        <span key={rail.id} data-layout-rail-state={rail.state}>
                          <i aria-hidden="true" />
                    ...
                    <div className="shield-pass289-layout-fix-list">
                      {layoutStabilitySentinelGate.fixes
                        .slice(0, 5)
                        .map((fix) => (
                          <span key={fix}>{fix}</span>
                        ))}
                    </div>
                    <div className="shield-pass289-layout-footer">
                      <code>{layoutStabilitySentinelGate.layoutId}</code>
                      <strong>{layoutStabilitySentinelGate.nextAction}</strong>
                      <small>
                        {layoutStabilitySentinelGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass290-disclosure-loom shield-pass290-disclosure-loom-${operatorOnlyReportFieldGate.status}`}
                    data-pass290-operator-only-report-field-gate
                  >
                    <div className="shield-pass290-disclosure-head">
                      <span>{operatorOnlyReportFieldGate.headline}</span>
                      <strong>{operatorOnlyReportFieldGate.trustBadge}</strong>
                    </div>
                    <p>{operatorOnlyReportFieldGate.operatorCue}</p>
                    <div className="shield-pass290-disclosure-rail-grid">
                      {operatorOnlyReportFieldGate.rails.map((rail) => (
                        <span
                          key={rail.id}
                          data-pass290-disclosure-tone={rail.tone}
                        >
                          <strong>{rail.label}</strong>
                          <b>{rail.value}</b>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass290-disclosure-lanes"
                      aria-label="operator-only report field separation"
                    >
                      {operatorOnlyReportFieldGate.lanes.map((lane) => (
                        <span
                          key={lane.id}
                          data-pass290-report-field-class={lane.fieldClass}
                        >
                          <strong>{lane.label}</strong>
                          <b>{lane.value}</b>
                          <small>
                            {lane.displayMode} · {lane.note}
                          </small>
                        </span>
                      ))}
                    </div>
                    <div className="shield-pass290-disclosure-split">
                      <div>
                        <span>customer preview</span>
                        {operatorOnlyReportFieldGate.customerLines.map(
                          (line) => (
                            <p key={line}>{line}</p>
                          ),
                        )}
                      ...
                      <div>
                        <span>operator vault</span>
                        {operatorOnlyReportFieldGate.operatorVaultLines.map(
                          (line) => (
                            <p key={line}>{line}</p>
                          ),
                        )}
                    ...
                    <div className="shield-pass290-disclosure-footer">
                      <code>{operatorOnlyReportFieldGate.disclosureId}</code>
                      <strong>{operatorOnlyReportFieldGate.nextAction}</strong>
                      <small>
                        {operatorOnlyReportFieldGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass291-replay-boundary shield-pass291-replay-boundary-${pdfBrowserReplayBoundaryGate.status}`}
                    data-pass291-pdf-browser-replay-boundary-gate
                  >
                    <div className="shield-pass291-replay-head">
                      <span>{pdfBrowserReplayBoundaryGate.headline}</span>
                      <strong>{pdfBrowserReplayBoundaryGate.trustBadge}</strong>
                    </div>
                    <p>{pdfBrowserReplayBoundaryGate.operatorCue}</p>
                    <div className="shield-pass291-replay-rail-grid">
                      {pdfBrowserReplayBoundaryGate.rails.map((rail) => (
                        <span
                          key={rail.id}
                          data-pass291-replay-tone={rail.tone}
                        >
                          <strong>{rail.label}</strong>
                          <b>{rail.value}</b>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass291-replay-stage-grid"
                      aria-label="PDF browser replay boundary stages"
                    >
                      {pdfBrowserReplayBoundaryGate.stages.map(
                        (stage, index) => (
                          <span
                            key={stage.id}
                            data-pass291-replay-stage={stage.state}
                            style={{ "--replay-index": index } as CSSProperties}
                          >
                            <i aria-hidden="true" />
                            <strong>{stage.label}</strong>
                            <b>{stage.value}</b>
                    ...
                    <div className="shield-pass291-replay-script">
                      <span>browser replay script</span>
                      {pdfBrowserReplayBoundaryGate.replayScript
                        .slice(0, 4)
                        .map((step) => (
                          <p key={step}>{step}</p>
                        ))}
                    </div>
                    <div className="shield-pass291-replay-preview">
                      {pdfBrowserReplayBoundaryGate.previewLines.map((line) => (
                        <span key={line}>{line}</span>
                      ))}
                    </div>
                    <div className="shield-pass291-replay-footer">
                      <code>{pdfBrowserReplayBoundaryGate.replayId}</code>
                      <button
                        type="button"
                        onClick={downloadPdfBrowserReplayPacket}
                      >
                        download replay packet
                      </button>
                      <strong>
                        {pdfReplayNotice === "downloaded"
                          ? "packet ready"
                          : pdfReplayNotice === "failed"
                            ? "packet failed"
                            : pdfBrowserReplayBoundaryGate.nextAction}
                      </strong>
                      <small>
                        {pdfBrowserReplayBoundaryGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass277-source-policy-gate shield-pass277-source-policy-gate-${sourcePolicyAllowlistGate.status}`}
                    data-pass277-source-policy-allowlist-gate
                  >
                    <div className="shield-pass277-source-policy-head">
                      <span>{sourcePolicyAllowlistGate.headline}</span>
                      <strong>{sourcePolicyAllowlistGate.trustBadge}</strong>
                    </div>
                    <p>{sourcePolicyAllowlistGate.operatorCue}</p>
                    <div className="shield-pass277-source-policy-grid">
                      {sourcePolicyAllowlistGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass277-source-policy-chip shield-pass277-source-policy-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass277-source-policy-lanes"
                      aria-label="source policy lanes"
                    >
                      {sourcePolicyAllowlistGate.lanes
                        .slice(0, 4)
                        .map((lane) => (
                          <span
                            key={lane.id}
                            data-pass277-source-policy-lane={lane.policy}
                          >
                            <strong>{lane.label}</strong>
                            <b>
                              {lane.sourceClass} · {lane.evidenceUse}
                            </b>
                            <small>{lane.note}</small>
                          </span>
                        ))}
                    </div>
                    <div className="shield-pass277-source-policy-next">
                      <span>next</span>
                      <strong>{sourcePolicyAllowlistGate.nextAction}</strong>
                      <small>
                        {sourcePolicyAllowlistGate.customerBoundary}
                      </small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass278-audit-vault shield-pass278-audit-vault-${durableAuditReceiptVault.status}`}
                    data-pass278-durable-audit-receipt-vault
                  >
                    <div className="shield-pass278-audit-vault-head">
                      <span>{durableAuditReceiptVault.headline}</span>
                      <strong>{durableAuditReceiptVault.trustBadge}</strong>
                    </div>
                    <p>{durableAuditReceiptVault.operatorCue}</p>
                    <div className="shield-pass278-audit-vault-grid">
                      {durableAuditReceiptVault.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass278-audit-vault-chip shield-pass278-audit-vault-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div
                      className="shield-pass278-audit-vault-receipts"
                      aria-label="durable audit receipt lanes"
                    >
                      {durableAuditReceiptVault.receipts.map((receipt) => (
                        <span
                          key={receipt.id}
                          data-pass278-audit-receipt={receipt.status}
                        >
                    ...
                    <div className="shield-pass278-audit-vault-next">
                      <span>next</span>
                      <strong>{durableAuditReceiptVault.nextAction}</strong>
                      <small>{durableAuditReceiptVault.customerBoundary}</small>
                    </div>
                  </div>

                  <div
                    className={`shield-pass273-exit-gate shield-pass273-exit-gate-${liquidityExitGate.status}`}
                    data-pass273-liquidity-exit-gate
                  >
                    <div className="shield-pass273-exit-head">
                      <span>{liquidityExitGate.headline}</span>
                      <strong>{liquidityExitGate.trustBadge}</strong>
                    </div>
                    <p>{liquidityExitGate.operatorCue}</p>
                    <div className="shield-pass273-exit-grid">
                      {liquidityExitGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass273-exit-chip shield-pass273-exit-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div className="shield-pass273-exit-next">
                      <span>next</span>
                      <strong>{liquidityExitGate.nextAction}</strong>
                    </div>
                  </div>

                  <div
                    className={`shield-pass272-holder-gate shield-pass272-holder-gate-${holderConcentrationGate.status}`}
                    data-pass272-holder-concentration-gate
                  >
                    <div className="shield-pass272-holder-head">
                      <span>{holderConcentrationGate.headline}</span>
                      <strong>{holderConcentrationGate.trustBadge}</strong>
                    </div>
                    <p>{holderConcentrationGate.operatorCue}</p>
                    <div className="shield-pass272-holder-grid">
                      {holderConcentrationGate.rails.map((rail) => (
                        <div
                          key={rail.id}
                          className={`shield-pass272-holder-chip shield-pass272-holder-chip-${rail.tone}`}
                        >
                          <span>{rail.label}</span>
                          <strong>{rail.value}</strong>
                          <small>{rail.note}</small>
                    ...
                    <div className="shield-pass272-holder-next">
                      <span>next</span>
                      <strong>{holderConcentrationGate.nextAction}</strong>
                    </div>
                  </div>
                </div>
              </details>
              ...
              <div className="shield-source-spine-panel hidden">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="shrink-0 rounded-full border border-white/[0.10] bg-black/[0.30] px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.12em] text-white/[0.50]">
                    {sourceSpineReadyCount}/{sourceSpineRows.length}{" "}
                    {sourceSpineCopy.readyLabel}
                  </span>
                </div>
                <div className="mt-3 grid gap-2">
                  {sourceSpineRows.map((row) => (
                    <div
                      key={row.id}
                      className={`shield-source-spine-row shield-source-spine-row-${row.tone}`}
                    >
                      <span className="shield-source-spine-dot" />
                      <div className="min-w-0">
                        <p className="font-mono text-[9px] uppercase tracking-[0.13em] text-white/[0.60]">
                          {row.label}
                        </p>
                        <p className="mt-1 truncate text-[11px] text-white/[0.42]">
                          {row.detail}
                        </p>
                        <p className="mt-1 font-mono text-[8px] uppercase tracking-[0.12em] text-white/[0.30]">
                          {sourceSpineCopy.freshness}: {row.freshness}
                        </p>
                      </div>
              ...
              <div
                className="shield-token-metric-grid-hidden"
                aria-hidden="true"
              >
                {[
                  ...
                  [
                    "confidence",
                    `${Math.round((result.confidence ?? 0.42) * 100)}%`,
                  ],
                ].map(([label, value]) => (
                  <div
                    key={label}
                    className="rounded-2xl border border-white/[0.08] bg-black/[0.22] p-3"
                  >
                    <p className="font-mono text-[9px] uppercase tracking-[0.14em] text-white/[0.32]">
                      {label}
                    </p>
              ...
              <div
                className="shield-token-review-tools-hidden"
                aria-hidden="true"
              >
                <div className="shield-operator-casefile mt-5 rounded-[1.35rem] border border-cyan-200/[0.12] bg-cyan-300/[0.035] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-cyan-100/[0.56]">
                        {ui.caseKicker} · {operatorCaseFile.evidenceStatus}
                      </p>
                      <h3 className="mt-1 text-sm font-semibold tracking-[-0.02em] text-white">
                        {ui.caseTitle}
                      </h3>
                    </div>
                    <span className="rounded-full border border-white/[0.08] bg-black/[0.26] px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.12em] text-velmere-gold">
                      {operatorCaseFile.caseId.slice(0, 12)}
                    </span>
                  </div>
                  <p className="mt-3 line-clamp-3 text-xs leading-5 text-white/[0.54]">
                    {operatorCaseFile.quickVerdict}
                  </p>
                  <div className="mt-3 grid gap-2 font-mono text-[9px] uppercase tracking-[0.12em] text-white/[0.42]">
                    <p>
                      <span className="text-velmere-gold/[0.76]">
                        {ui.caseNext}:
                      </span>{" "}
                    ...
                    <p>
                      ...
                      {operatorCaseFile.blockers.length}
                    </p>
                ...
                <div className="shield-evidence-draft mt-4 rounded-[1.35rem] border border-velmere-gold/[0.14] bg-velmere-gold/[0.045] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-velmere-gold/[0.70]">
                        evidence draft · {evidenceReportDraft.exportStatus}
                      </p>
                      <h3 className="mt-1 text-sm font-semibold tracking-[-0.02em] text-white">
                        Source ledger report
                      </h3>
                    </div>
                    <span className="rounded-full border border-white/[0.08] bg-black/[0.26] px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.12em] text-white/[0.56]">
                      {evidenceReportDraft.mode.replace("_", " ")}
                    </span>
                  </div>
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    {[
                      ...
                      [
                        "missing",
                        evidenceReportDraft.missingDataAppendix.length,
                      ],
                      ["sections", evidenceReportDraft.sections.length],
                    ].map(([label, value]) => (
                      <div
                        key={label}
                        className="rounded-xl border border-white/[0.07] bg-black/[0.20] p-2"
                      >
                  ...
                  <div className="mt-3 grid gap-2 font-mono text-[9px] uppercase tracking-[0.12em] text-white/[0.42]">
                    <p className="truncate">
                      <span className="text-velmere-gold/[0.76]">report:</span>{" "}
                      {evidenceReportDraft.reportId}
                    </p>
                    <p className="line-clamp-2">
                      <span className="text-velmere-gold/[0.76]">next:</span>{" "}
                      {evidenceReportDraft.sections.find(
                        (section) => section.status === "blocked",
                      )?.body ?? evidenceReportDraft.legalNote}
                    </p>
                  </div>
                  <div className="shield-evidence-export-manifest mt-3 rounded-[1rem] border border-white/[0.08] bg-black/[0.18] p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="font-mono text-[8px] uppercase tracking-[0.16em] text-white/[0.36]">
                          {ui.evidenceExportTitle}
                        </p>
                        <p className="mt-1 text-[11px] leading-5 text-white/[0.44]">
                          {ui.evidenceExportBody}
                        </p>
                      </div>
                      <span className="shrink-0 rounded-full border border-white/[0.08] px-2 py-1 font-mono text-[8px] uppercase tracking-[0.12em] text-white/[0.42]">
                        {evidenceExportManifest.manifestMode.replaceAll(
                          "_",
                          " ",
                        )}
                      </span>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <button
                        type="button"
                        className="rounded-full border border-velmere-gold/[0.22] bg-velmere-gold/[0.08] px-3 py-2 font-mono text-[9px] uppercase tracking-[0.14em] text-velmere-gold transition hover:bg-velmere-gold/[0.12]"
                      ...
                      <button
                        type="button"
                        className="rounded-full border border-white/[0.10] bg-white/[0.035] px-3 py-2 font-mono text-[9px] uppercase tracking-[0.14em] text-white/[0.62] transition hover:bg-white/[0.06]"
                        onClick={copyEvidenceManifest}
                      >
                        {evidenceExportNotice === "copied"
                          ? ui.evidenceCopied
                          : evidenceExportNotice === "failed"
                            ? "Retry"
                            : ui.evidenceCopy}
                      </button>
                    </div>
