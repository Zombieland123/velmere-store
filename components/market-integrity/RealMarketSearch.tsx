...
export default function RealMarketSearch() {
  ...
  return (
    <div className="shield-real-search" data-real-market-search="true">
      <div className="shield-real-search-input-row">
          ...
          onKeyDown={(event) => {
            ...
            if (event.key === "ArrowDown") {
              ...
              setActiveIndex((current) =>
                Math.min(
                  current + 1,
                  Math.max(0, Math.min(results.length, 12) - 1),
                ),
              );
            }
            ...
            if (event.key === "Enter" && open && results[activeIndex]) {
              event.preventDefault();
              setQuery(results[activeIndex].symbol);
              setOpen(false);
            }
          }}
        ...
        <span>{providerMode === "catalog_live" ? "global catalog" : "curated fallback"}</span>
      </div>
