...
const toneClasses = {
  locked: "border-white/10 bg-white/[0.035]",
  warning: "border-velmere-gold/25 bg-velmere-gold/[0.055]",
        ...
        <article key={key} className={cn("rounded-2xl border p-6", toneClasses[tone])}>
          <div className="flex min-h-[128px] items-start justify-between gap-5">
            <div>
              ...
              <p className="mt-4 text-sm leading-6 text-white/74">{t(`${key}.value`)}</p>
              <span className="mt-5 inline-flex min-h-8 items-center rounded-full border border-white/10 px-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-white/48">
                {t(`${key}.chip`)}
              </span>
            </div>
            <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 bg-black/18">
