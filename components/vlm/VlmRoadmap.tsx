  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 md:p-8">
      ...
      <div className="relative mt-10">
        <div className="absolute left-5 top-0 hidden h-full w-px bg-gradient-to-b from-velmere-gold/70 via-white/10 to-transparent md:block" />
            ...
            return (
              <motion.article
                ...
                className={`relative grid gap-4 rounded-2xl border p-5 md:grid-cols-[auto_0.8fr_1fr] md:items-center md:pl-16 ${
                  active
                    ? "border-velmere-gold/28 bg-velmere-gold/[0.065]"
                    : step.status === "locked"
                      ? "border-white/10 bg-black/18 opacity-72"
                      : "border-white/10 bg-black/24"
                }`}
              >
                <span className="absolute left-0 top-5 hidden h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-black text-velmere-gold md:inline-flex">
                ...
                <p className="font-sans text-xs font-semibold uppercase tracking-[0.16em] text-white/42 md:text-right">
