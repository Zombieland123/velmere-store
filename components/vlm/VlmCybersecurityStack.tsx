...
export default function VlmCybersecurityStack() {
  ...
  return (
    <section className="mx-auto w-full max-w-6xl scroll-mt-28 px-6 py-12 md:px-12 md:py-16">
      <div className="rounded-[2rem] border border-white/10 bg-[#111111] p-6 md:p-10">
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-2">
          <article className="rounded-3xl border border-white/10 bg-black/24 p-5 md:p-6">
            <p className="font-sans text-[10px] font-semibold uppercase tracking-[0.24em] text-white/42">
              {t("visualLayer.title")}
            </p>
            <p className="mt-3 font-sans text-sm leading-7 text-white/56">{t("visualLayer.body")}</p>
            <div className="mt-5 grid gap-2 sm:grid-cols-2">
              {visualItems.map((item) => (
                <div key={item} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                  <p className="font-sans text-xs font-semibold uppercase tracking-[0.16em] text-white/70">
          ...
          <article className="rounded-3xl border border-velmere-gold/20 bg-velmere-gold/[0.055] p-5 md:p-6">
            <p className="font-sans text-[10px] font-semibold uppercase tracking-[0.24em] text-velmere-gold">
              {t("realLayer.title")}
            </p>
            <p className="mt-3 font-sans text-sm leading-7 text-white/62">{t("realLayer.body")}</p>
            <div className="mt-5 grid gap-2 sm:grid-cols-2">
              {realItems.map((item) => (
                <div key={item} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                  <p className="font-sans text-xs font-semibold uppercase tracking-[0.16em] text-white/72">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {stack.map(({ key, icon: Icon }) => (
            <article key={key} className="rounded-2xl border border-white/10 bg-black/24 p-5">
              ...
              <p className="mt-3 font-sans text-xs leading-6 text-white/50">{t(`cards.${key}.body`)}</p>
            </article>
          ))}
        </div>
      </div>
