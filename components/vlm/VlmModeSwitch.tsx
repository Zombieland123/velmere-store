...
export default function VlmModeSwitch({ inline = false }: { inline?: boolean }) {
  ...
  const control = (
    <div
      role="tablist"
      aria-label={t("aria")}
      className={`inline-flex rounded-full border border-white/10 bg-[#121214]/88 p-1 shadow-[0_18px_60px_rgba(0,0,0,0.48)] backdrop-blur-2xl ${inline ? "origin-left scale-[0.86] 2xl:scale-[0.92]" : ""}`}
    >
      {[
        ...
        return (
          <Link
            ...
            className={`min-h-10 ${inline ? "min-w-[4.6rem] px-3" : "min-w-24 px-4 md:min-w-28 md:px-5"} rounded-full text-center font-sans text-[9px] font-black uppercase tracking-[0.22em] transition-colors active:scale-95 ${
              active
                ? item.key === "pro"
