...
export default function VlmAccessVisual() {
  ...
  return (
    <section className="mobile-scroll-safe-canvas relative aspect-square max-h-[360px] w-full overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_50%_42%,rgba(200,169,106,0.16),rgba(255,255,255,0.03)_35%,rgba(0,0,0,0)_70%)] text-white touch-pan-y md:aspect-[4/3] md:max-h-none lg:aspect-square">
        ...
        {["evm", "sol", "sui"].map((network, index) => (
          <motion.span
            ...
            className={`rounded-full border px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] ${
              network === "evm"
                ? "border-velmere-gold/35 bg-velmere-gold/[0.08] text-velmere-gold"
                : "border-white/12 bg-black/36 text-white/52"
            }`}
      ...
      >
        ...
        <g transform="translate(70 80) scale(1.04)">
          ...
          <motion.path
            ...
            initial={false}
            animate={reducedMotion ? undefined : { pathLength: [0.58, 1, 0.58], opacity: [0.45, 0.92, 0.45] }}
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 border-t border-white/10 bg-black/62 p-4 backdrop-blur-xl md:p-5">
        <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-center">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.28em] text-velmere-gold">{t("title")}</p>
            <p className="mt-2 max-w-2xl text-xs leading-6 text-white/56 md:text-sm">{t("body")}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex h-10 items-center gap-2 rounded-full border border-white/12 px-4 text-[10px] uppercase tracking-[0.16em] text-white/52">
              <LockKeyhole className="h-3.5 w-3.5 text-velmere-gold" aria-hidden="true" />
              {t("notDeployed")}
            </span>
            <span className="hidden h-10 items-center gap-2 rounded-full border border-white/12 px-4 text-[10px] uppercase tracking-[0.16em] text-white/52 sm:inline-flex">
