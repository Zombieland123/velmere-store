...
export default function VlmWalletSidePanel() {
  ...
  return (
    <div className="space-y-6">
      <p className="font-sans text-sm leading-7 text-white/58">{t("safetyNote")}</p>

      <div className="rounded-2xl border border-white/5 bg-black/24 p-5">
        <p className="font-sans text-[10px] uppercase tracking-[0.18em] text-white/40">{t("statusLabel")}</p>
        <p className="mt-2 font-sans text-sm text-white/72">{t(`states.${effectiveState}`)}</p>
        <p className="mt-3 font-sans text-xs leading-6 text-white/48">{helper}</p>
        <span className="mt-4 inline-flex min-h-8 items-center rounded-full border border-[#d4af37]/25 px-3 text-[10px] uppercase tracking-[0.14em] text-[#d4af37]">
          {t("registryBadge")}
        </span>
      </div>
      ...
      <button
        ...
        className="flex min-h-11 w-full items-center justify-between rounded-full border border-white/10 px-5 text-[10px] font-semibold uppercase tracking-[0.16em] text-white/62 disabled:opacity-40"
      >
        MetaMask
      ...
      <div>
        <label className="font-sans text-[10px] uppercase tracking-[0.18em] text-white/40">{t("addressLabel")}</label>
        <input
          value={manualAddress}
          onChange={(event) => setManualAddress(event.target.value)}
          placeholder={t("addressPlaceholder")}
          spellCheck={false}
          className="mt-2 min-h-11 w-full rounded-full border border-white/10 bg-black/24 px-4 font-mono text-sm text-white outline-none"
        />
        <p
          className={`mt-2 text-xs leading-6 ${
            manualAddressState === "valid" ? "text-[#d4af37]" : manualAddressState === "invalid" ? "text-red-200/80" : "text-white/42"
          }`}
        >
      ...
      <p className="flex items-start gap-2 text-xs leading-6 text-white/44">
        {state === "connecting" ? (
