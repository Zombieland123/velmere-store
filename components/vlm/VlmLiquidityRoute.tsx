...
export default function VlmLiquidityRoute({ compact = false }: { compact?: boolean } = {}) {
  ...
  return (
    <section className={compact ? "" : `${SECTION_NARROW} scroll-mt-28 py-12 md:py-16`}>
      <div className={`rounded-[2rem] border border-white/5 bg-white/[0.035] ${compact ? "p-6" : "p-6 md:p-8"}`}>
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          <div>
            ...
            <p className="mt-5 rounded-2xl border border-white/10 bg-black/24 p-4 font-sans text-xs leading-6 text-white/46">
              {t("clarityNote")}
            </p>
          </div>
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {examples.map((item) => (
                <article key={item} className="rounded-2xl border border-white/10 bg-white/[0.025] p-4">
                  <p className="font-sans text-[10px] font-semibold uppercase tracking-[0.16em] text-white/38">
                    {t(`examples.${item}.label`)}
                  </p>
                  <p className="mt-2 font-sans text-sm leading-6 text-white/64">{t(`examples.${item}.value`)}</p>
                </article>
              ))}
            </div>

            <div className="rounded-2xl border border-velmere-gold/25 bg-velmere-gold/[0.065] p-4">
              <div className="flex gap-3">
                <AlertTriangle className="mt-1 h-4 w-4 shrink-0 text-velmere-gold" aria-hidden="true" />
                <p className="font-sans text-xs leading-6 text-white/58">{t("warning")}</p>
              </div>
            </div>
          </div>
        </div>
