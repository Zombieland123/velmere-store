    ...
    <div className="rounded-2xl border border-white/10 bg-black/22 p-4">
      <div className="flex flex-wrap gap-2">
        {formulas.map((formula) => (
          <span
            key={formula}
            className="rounded-full border border-white/10 bg-white/[0.035] px-3 py-2 font-mono text-[11px] leading-5 text-white/62"
          >
            {formula}
          </span>
        ))}
      </div>
      <p className="mt-3 font-sans text-xs leading-6 text-white/44">{t("disclaimer")}</p>
    </div>
  );
}
