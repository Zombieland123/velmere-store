...
export default function AccessFormulaVisual({ compact = false, active = true }: { compact?: boolean; active?: boolean } = {}) {
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#121212]/90 p-5 md:p-8">
      ...
      <div className="mt-5 rounded-2xl border border-white/5 bg-black/20 p-4">
        <p className="font-mono text-xs text-white/55">AccessScore = f(address, balance, network, rules)</p>
      </div>

      <div className="mt-5 grid grid-cols-2 gap-3">
        {inputs.map((key) => (
          <div key={key} className="min-w-0 rounded-2xl border border-white/5 bg-black/20 p-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/38">{t(`labels.${key}`)}</p>
      ...
      <div className="mt-8 flex flex-col items-center gap-5">
        <div
          className="flex h-24 w-24 shrink-0 flex-col items-center justify-center rounded-full border border-[#d4af37]/30 bg-[#0a0a0a] text-center"
          aria-hidden={!active}
        >
          <LockKeyhole className="h-5 w-5 text-[#d4af37]" />
          <span className="mt-2 text-[9px] font-semibold uppercase tracking-[0.14em] text-white/54">{t("gate")}</span>
        </div>

        <div className="w-full max-w-md rounded-2xl border border-[#d4af37]/20 bg-[#d4af37]/[0.06] p-4">
          ...
          <p className="mt-2 text-xs leading-6 text-white/58">{t(`reasons.${reason}`)}</p>
        </div>
      </div>

      <p className="mt-5 border-t border-white/5 pt-4 text-xs leading-6 text-white/44">{t("disclaimer")}</p>
      {compact ? (
        <p className="mt-2 text-[10px] uppercase tracking-[0.14em] text-white/32">{t("internalNote")}</p>
      ) : null}
    </div>
  );
}
