...
export default function BajakProtocolMini() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-14 md:px-12 md:py-20">
      <motion.div {...entrance} className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-6 md:p-8">
        <div className="grid gap-7 lg:grid-cols-[0.78fr_1.22fr] lg:items-center">
          <div>
            ...
            <p className="mt-5 max-w-2xl font-sans text-sm leading-7 text-white/58">{t("subtitle")}</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {miniCards.map(({ key, icon: Icon }) => (
              <article key={key} className="rounded-2xl border border-white/10 bg-black/24 p-5">
                ...
                <p className="mt-4 font-sans text-[10px] font-semibold uppercase tracking-[0.2em] text-white/42">{t(`cards.${key}.label`)}</p>
                <p className="mt-2 font-sans text-xs leading-6 text-white/58">{t(`cards.${key}.body`)}</p>
              </article>
            ))}
          </div>
        </div>
        <div className="mt-6 rounded-full border border-velmere-gold/20 bg-velmere-gold/[0.055] px-5 py-3 font-sans text-[10px] uppercase tracking-[0.2em] text-velmere-gold/80">
