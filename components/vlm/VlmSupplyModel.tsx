      ...
      <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {sections.map((section) => (
            <article key={section} className="rounded-2xl border border-white/10 bg-black/24 p-5">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/66">
                {t(`sections.${section}.title`)}
              </p>
              <p className="mt-3 text-sm leading-6 text-white/48">{t(`sections.${section}.body`)}</p>
            </article>
          ))}
        </div>
      </div>
