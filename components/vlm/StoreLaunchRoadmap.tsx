  ...
  return (
    <section className={`${SECTION_NARROW} py-4`}>
      <div className="mb-10">
        ...
        <p className="mt-4 max-w-2xl font-sans text-sm leading-7 text-white/56">{t("body")}</p>
      </div>
      <div className="grid gap-6 lg:grid-cols-3">
        {phases.map((phase) => (
          <article key={phase} className="rounded-[2rem] border border-white/5 bg-white/[0.03] p-8 md:p-10">
            ...
            <ul className="mt-6 space-y-3">
              {(t.raw(`phases.${phase}.items`) as string[]).map((item) => (
                <li key={item} className="flex gap-3 font-sans text-sm leading-6 text-white/58">
