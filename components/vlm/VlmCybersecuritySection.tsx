    ...
    <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 md:p-8">
      ...
      <div className="grid gap-3 md:grid-cols-5">
        {principles.map((principle) => (
          <article key={principle} className="rounded-2xl border border-white/10 bg-black/24 p-5 md:col-span-1">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-velmere-gold">
              {t(`principles.${principle}.title`)}
            </p>
            <p className="mt-3 text-sm leading-6 text-white/48">{t(`principles.${principle}.body`)}</p>
          </article>
        ))}
      </div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {checklist.map((item) => (
          <div key={item} className="flex min-h-16 items-center gap-3 rounded-2xl border border-white/10 bg-black/24 px-4 py-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-white/40">{t(`checklist.${item}.status`)}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-white/10 bg-black/24 p-5">
        <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/64">{t("riskTitle")}</p>
        <div className="mt-4 flex flex-wrap gap-2">
          {risks.map((risk) => (
            <span
              key={risk}
              className="inline-flex min-h-9 items-center gap-2 rounded-full border border-white/10 px-3 text-[10px] uppercase tracking-[0.14em] text-white/46"
            >
              <CheckCircle2 className="h-3.5 w-3.5 text-white/28" aria-hidden="true" />
              {t(`risks.${risk}`)}
            </span>
          ))}
        </div>
