...
export default function VlmActionBar({ onAction }: VlmActionBarProps) {
  ...
  return (
    <div className="no-scrollbar w-full overflow-x-auto pb-1">
      <div className="inline-flex min-w-max gap-1 rounded-full border border-white/10 bg-white/[0.035] p-1">
        {actions.map((action) => (
          <button
            key={action.key}
            type="button"
            onClick={() => onAction(action.key)}
            className="min-h-[44px] shrink-0 rounded-full px-5 font-sans text-[10px] font-semibold uppercase tracking-[0.16em] text-[#FFFFF0]/72 transition-all duration-200 hover:bg-white/[0.07] hover:text-[#FFFFF0] focus:outline-none focus:ring-2 focus:ring-[#d4af37]/20"
          >
            {t(action.labelKey)}
          </button>
        ))}
