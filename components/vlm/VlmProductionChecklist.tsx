  ...
  return (
    <section className={compact ? "py-0" : `${SECTION_NARROW} py-8 md:py-12`}>
      <div className={compact ? "mb-5 max-w-2xl" : "mb-8 max-w-2xl"}>
        ...
        <p className="mt-4 font-sans text-xs leading-6 text-white/56 md:text-sm md:leading-7">{t("body")}</p>
      </div>
      <div className="grid gap-3 md:grid-cols-3">
        {phases.map((phase) => (
          <article key={phase} className="rounded-none border border-white/5 bg-white/[0.03] p-4 md:p-8">
            ...
            <ul className="mt-5 space-y-2.5">
              {(t.raw(`phases.${phase}.items`) as string[]).map((item) => (
                <li key={item} className="flex gap-2.5 font-mono text-[11px] leading-6 text-white/58 md:text-sm">
