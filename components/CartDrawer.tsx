...
export default function CartDrawer() {
  ...
  return (
    <Drawer.Root open={isOpen} onOpenChange={(open) => !open && closeCart()}>
      <Drawer.Portal>
        <Drawer.Overlay className="fixed inset-0 z-[100] bg-black/[0.55] backdrop-blur-xl" />
        <Drawer.Content className="fixed bottom-2 right-2 top-2 z-[110] flex h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] flex-col overflow-hidden rounded-2xl border border-white/[0.10] bg-[#1A1A1C] text-white shadow-2xl shadow-black/[0.50] outline-none md:bottom-4 md:right-4 md:top-4 md:h-[calc(100dvh-2rem)] md:w-[min(31rem,calc(100vw-2rem))]">
          <div className="mx-auto mt-3 h-1 w-14 rounded-full bg-white/[0.20] md:hidden" aria-hidden="true" />
          <header className="flex items-center justify-between border-b border-white/[0.10] px-4 py-4 md:px-5 md:py-5">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.24em] text-velmere-gold/[0.80]">{t("kicker")}</p>
              <Drawer.Title className="mt-2 font-serif text-2xl tracking-[0.08em] text-white md:text-3xl">{t("orderBook")}</Drawer.Title>
              <p className="mt-1 font-mono text-[9px] uppercase tracking-[0.2em] text-white/[0.35]">{itemCount} {t("units")}</p>
            </div>
            <Drawer.Close asChild>
              <button type="button" aria-label={common("close")} className="flex h-11 w-11 items-center justify-center rounded-full border border-white/[0.10] bg-black/[0.20] text-white/[0.62] transition-colors hover:border-white/[0.25] hover:text-white active:scale-95">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto scroll-touch px-0 py-0 luxury-scrollbar">
            {items.length === 0 ? (
              <div className="flex h-full min-h-[18rem] flex-col items-center justify-center px-6 py-8 text-center">
                <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-white/[0.10] bg-[#242428]">
                  <ShoppingBag className="h-7 w-7 text-white/[0.38]" aria-hidden="true" />
                </div>
                <p className="mt-5 max-w-[15rem] font-mono text-[10px] uppercase leading-6 tracking-[0.18em] text-white/[0.42] md:max-w-xs md:text-[11px] md:leading-7 md:tracking-[0.22em]">{t("emptyLong")}</p>
                <Drawer.Close asChild>
                  <Link href="/shop" className="mt-8 inline-flex min-h-12 items-center rounded-full border border-white/[0.12] bg-[#242428] px-6 font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-white/[0.70] transition-colors hover:border-white/[0.25] hover:text-white active:scale-95">
                    {t("continueShopping")}
                  </Link>
                </Drawer.Close>
              </div>
              ...
              <ul className="divide-y divide-white/[0.05]">
                {items.map((item, index) => (
                  <li key={`${item.id}-${item.size}`} className="grid gap-4 border-b border-white/[0.05] px-5 py-5">
                    <div className="grid grid-cols-[5rem_minmax(0,1fr)] gap-4">
                      <div className="relative h-24 overflow-hidden rounded-xl border border-white/[0.10] bg-black/[0.40]">
                        {item.image ? <Image src={item.image} alt={item.name} fill sizes="80px" className="object-cover contrast-105" /> : <div className="h-full w-full bg-white/[0.05]" />}
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {SIZES.map((size) => (
                        <button key={size} type="button" onClick={() => { haptic(); updateSize(item.id, item.size, size); }} className={`flex h-9 min-w-9 items-center justify-center rounded-full border px-2 font-mono text-[10px] transition-colors active:scale-95 ${item.size === size ? "border-velmere-gold bg-velmere-gold text-black" : "border-white/[0.12] bg-black/[0.20] text-white/[0.52] hover:border-white/[0.25] hover:text-white"}`}>
                          {size}
                        </button>
                      ))}
                      <button type="button" aria-label="Decrease quantity" onClick={() => removeItem(item.id, item.size)} className="ml-auto flex h-9 w-9 items-center justify-center rounded-full border border-white/[0.10] text-white/[0.50] hover:text-white active:scale-95"><Minus className="h-3 w-3" /></button>
                      <span className="flex h-9 min-w-9 items-center justify-center rounded-full border border-white/[0.10] font-mono text-[10px] text-white/[0.70]">{item.quantity}</span>
                      <button type="button" aria-label="Increase quantity" onClick={() => { haptic(); addItem({ ...item, quantity: 1 }); }} className="flex h-9 w-9 items-center justify-center rounded-full border border-white/[0.10] text-white/[0.50] hover:text-white active:scale-95"><Plus className="h-3 w-3" /></button>
                    </div>
                  </li>
                ))}
              </ul>
          ...
          <footer className="max-h-[50dvh] shrink-0 overflow-y-auto border-t border-white/[0.10] bg-[#151517] px-4 py-4 safe-pb luxury-scrollbar md:max-h-none md:overflow-visible md:px-5 md:py-5">
            {items.length === 0 ? (
              <div className="flex items-center justify-between gap-4 rounded-xl border border-white/[0.10] bg-[#202024] px-4 py-3 font-mono text-[9px] uppercase tracking-[0.16em] text-white/[0.40]">
                <span>{t("noAllocation")}</span>
                <span className="text-[#c8a96a]/[0.70]">0.00 EUR</span>
              </div>
            ) : (
              <>
                <div className="grid gap-2 font-mono text-[9px] uppercase tracking-[0.16em] text-white/[0.38] sm:grid-cols-2">
                  <span>{trust("securePayment")}</span><span>{trust("trackedShipping")}</span><span>{trust("madeAfterOrder")}</span><span>{trust("support")}</span>
                </div>
                <div className="mt-4 space-y-2 border-y border-white/[0.10] py-3 font-mono text-[10px] uppercase tracking-[0.16em] text-white/[0.42] md:mt-5 md:py-4">
                  <div className="flex items-center justify-between gap-3"><span>{t("netPrice")}</span><span className="tabular-nums text-white/[0.72]">{formatMoney({ amount: netAmount, currency }, locale)}</span></div>
                  <div className="flex items-center justify-between gap-3"><span>{t("vat")}</span><span className="tabular-nums text-white/[0.72]">{formatMoney({ amount: vatAmount, currency }, locale)}</span></div>
                  <div className="flex items-center justify-between gap-3 pt-2 text-white/[0.70]"><span>{t("grossPrice")}</span><span className="tabular-nums text-white">{formatMoney({ amount: subtotal, currency }, locale)}</span></div>
                </div>
                ...
                {checkoutError ? <p className="mt-3 rounded-xl border border-velmere-gold/[0.20] bg-velmere-gold/[0.08] p-3 font-mono text-[10px] leading-5 text-white/[0.64]">{checkoutError}</p> : null}
              </>
            )}
          </footer>
        </Drawer.Content>
