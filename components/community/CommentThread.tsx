...
export default function CommentThread({ comments, draft, onDraftChange, onSubmit, labels }: CommentThreadProps) {
  ...
  return (
    <section className="flex h-full min-h-0 flex-col border-t border-white/[0.10] pt-5">
      <div className="sticky top-0 z-20 mb-4 flex items-center justify-between gap-4 bg-[#19191B]/[0.95] pb-3 backdrop-blur-xl">
      ...
      <div className="sticky top-12 z-20 mb-5 flex gap-2 rounded-2xl border border-velmere-gold/[0.20] bg-[#202024]/[0.96] p-3 shadow-[0_16px_60px_rgba(0,0,0,0.30)] backdrop-blur-xl">
        <input
          value={draft}
          onChange={(event) => onDraftChange(event.target.value)}
          placeholder={labels.placeholder}
          className="min-h-11 min-w-0 flex-1 rounded-full border border-white/[0.10] bg-white/[0.045] px-4 text-[16px] text-white outline-none placeholder:text-white/[0.30] focus:border-velmere-gold/[0.40]"
        />
      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto pr-1 luxury-scrollbar">
            ...
            <article key={comment.id} className="relative grid grid-cols-[2.5rem_1fr] gap-3">
              ...
              <div className="min-w-0">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <button type="button" className="ml-auto inline-flex h-8 w-8 items-center justify-center rounded-full text-white/[0.35] luxury-hover hover:bg-white/[0.04] hover:text-white" aria-label="Comment menu">
