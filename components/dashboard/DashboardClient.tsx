    ...
    <main className="min-h-[100dvh] bg-[#080809] pt-28 text-white md:pt-32">
      <div className="mx-auto grid w-full max-w-none gap-4 px-4 pb-28 md:px-8 lg:grid-cols-[18rem_minmax(0,1fr)]">
        ...
        <section className="min-w-0 rounded-2xl border border-white/10 bg-[#111113] p-4 shadow-2xl shadow-black/40 md:p-6">
          ...
          {active === "overview" && <div className="mt-6 space-y-4"><div className="grid gap-4 md:grid-cols-3"><Field label="Member rank" value="Private build" /><Field label="Wallet" value={walletUi.connected ? walletUi.fullAddress : "Optional binding"} /><Field label="Order engine" value="Stripe session pipeline armed" /><Field label="VLM balance" value={walletUi.tokenBalanceLabel || "0.00 VLM"} /><Field label="Security" value="2FA setup pending" /><Field label="Profile" value="Avatar and bio editor ready" /></div><div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]"><section className="rounded-2xl border border-white/10 bg-[#161618] p-5"><p className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#c8a96a]">Member timeline</p><div className="mt-5 space-y-3 font-mono text-[10px] uppercase tracking-[0.14em] text-white/48"><p>[ ACCESS ] :: SESSION_ACTIVE</p><p>[ ORDERS ] :: AWAITING_FIRST_CHECKOUT</p><p>[ SQUARE ] :: READ_PUBLIC / POST_AFTER_LOGIN</p></div></section><section className="rounded-2xl border border-[#c8a96a]/20 bg-[#c8a96a]/[0.06] p-5"><p className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#c8a96a]">Client relation</p><p className="mt-4 text-sm leading-7 text-white/58">This console is account-first. Web3 is treated as an optional access binding, not a checkout requirement.</p></section></div></div>}
