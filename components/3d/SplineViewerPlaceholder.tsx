  ...
  default: function EmptySplineScene({ scene, label }: SplineViewerPlaceholderProps) {
    return (
      <div
        ...
        className="relative h-full min-h-[280px] w-full overflow-hidden rounded-[2rem] border border-white/10 bg-[#050505]"
      >
        ...
        <div className="absolute left-1/2 top-1/2 h-20 w-20 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#d4af37]/30 bg-[#d4af37]/10 shadow-[0_0_60px_rgba(212,175,55,0.24)]" />
        <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between gap-4">
          ...
          <span className="rounded-full border border-white/10 px-3 py-2 font-mono text-[10px] uppercase tracking-[0.16em] text-white/44">
            Lazy
          </span>
        </div>
      </div>
...
export default function SplineViewerPlaceholder(props: SplineViewerPlaceholderProps) {
  return (
    <Suspense
      fallback={
        <div className={`min-h-[280px] rounded-[2rem] border border-white/10 bg-white/[0.035] ${props.className ?? ""}`} />
      }
    >
