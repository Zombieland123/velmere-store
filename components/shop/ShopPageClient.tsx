  ...
  return (
    <main className="min-h-[100dvh] bg-velmere-black text-white">
      <LuxurySection className="py-28 md:py-36">
        <div className="mb-12 grid gap-8 lg:grid-cols-12 lg:items-end">
          <div className="max-w-3xl lg:col-span-8">
            ...
            <p className="mt-6 text-sm leading-7 text-white/[0.58] md:text-base">{matrix.body}</p>
          </div>
          <div className="lg:col-span-4 lg:text-right">
            <span className="inline-flex min-h-12 items-center justify-center rounded-full border border-white/[0.12] bg-white/[0.035] px-5 text-xs font-semibold uppercase tracking-[0.14em] text-velmere-muted">{matrix.filter}</span>
          </div>
        </div>
        ...
        <motion.div initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.055 } } }} className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:gap-5">
          {visibleSlots.map((slot, index) => {
            ...
            return (
              <motion.article key={String(slot)} variants={{ hidden: { opacity: 0, y: 18 }, show: { opacity: 1, y: 0 } }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="group relative min-h-[25rem] overflow-hidden rounded-[2rem] border border-white/[0.08] bg-[linear-gradient(145deg,rgba(255,255,255,0.045),rgba(255,255,255,0.012)_48%,rgba(212,175,55,0.035))] p-5 shadow-[0_28px_90px_rgba(0,0,0,0.26)]">
                ...
                <div className="relative flex h-full flex-col justify-between">
                  <div className="aspect-[4/5] rounded-[1.35rem] border border-white/[0.08] bg-black/[0.25] p-5">
                  ...
                  <div className="pt-5">
                    ...
                    <h3 className="mt-3 font-serif text-2xl text-white/[0.82]">{String(slot)}</h3>
      ...
      <LuxurySection className="py-16 md:py-24">
        ...
        >
          <div>
            <p className="luxury-kicker text-velmere-gold/[0.70]">{t("archiveKicker")}</p>
            <h2 className="mt-4 font-serif text-4xl leading-tight text-white md:text-5xl">{t("archiveTitle")}</h2>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-white/[0.58]">{t("archiveBody")}</p>
          </div>
          <Link
            href="/vlm-token"
