...
export default function ProductLaunchChecklist() {
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-6 md:p-8">
      ...
      <ul className="mt-6 space-y-2 text-sm leading-6 text-white/62">
      ...
      {products.length > 0 ? (
        <div className="mt-8 space-y-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/40">{t("sampleTitle")}</p>
          {products.map((product) => {
            const readiness = getProductLaunchReadiness(product);
            return (
              <div key={product.id} className="rounded-2xl border border-white/10 bg-black/24 p-4">
