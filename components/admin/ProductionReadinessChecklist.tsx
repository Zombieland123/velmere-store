    ...
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-6 md:p-8">
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-2">
        {Array.from(groups.entries()).map(([group, items]: [string, ReadinessItem[]]) => (
          <div key={group} className="rounded-2xl border border-white/10 bg-black/24 p-4">
            <h3 className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/72">
            ...
            <ul className="mt-4 space-y-2">
              {items.map((item) => (
                <li key={item.id} className="flex items-center justify-between gap-3 text-xs">
                  ...
                  <span className={item.present ? "text-[#d4af37]" : "text-white/30"}>
