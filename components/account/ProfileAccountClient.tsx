...
export default function ProfileAccountClient() {
  ...
  return (
    <section className="mt-10 overflow-hidden rounded-[2rem] border border-white/[0.10] bg-black/[0.28]">
      <div className="grid gap-6 border-b border-white/[0.10] p-6 md:grid-cols-[auto_1fr_auto] md:items-center md:p-7">
        <div className="flex h-16 w-16 items-center justify-center rounded-full border border-velmere-gold/[0.30] bg-velmere-gold/[0.10] font-serif text-3xl text-velmere-gold">
          {profile.displayName.slice(0, 1)}
        </div>
        <div>
          <p className="font-sans text-[10px] font-black uppercase tracking-[0.24em] text-velmere-gold/[0.80]">{t("kicker")}</p>
        ...
        <button
          ...
          className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-white/[0.10] px-5 font-sans text-[10px] font-black uppercase tracking-[0.18em] text-white/[0.62] luxury-hover transition-transform hover:border-white/[0.25] hover:text-white active:scale-95"
        >
      ...
      {isLoading ? (
        <div className="grid gap-4 p-6 md:grid-cols-3 md:p-7">
          {[0, 1, 2].map((item) => (
            <div key={item} className="h-28 rounded-2xl border border-white/[0.10] bg-white/[0.035] p-4">
      ...
      {editing ? (
        <div className="border-t border-white/[0.10] p-6 md:p-7">
          <div className="grid gap-5 md:grid-cols-2">
            <label className="block scroll-mt-28">
              ...
              <input
                ...
                className="mt-3 min-h-12 w-full rounded-full border border-white/[0.10] bg-black/[0.40] px-5 text-sm text-white outline-none placeholder:text-white/[0.24] disabled:opacity-45"
              />
            </label>
            <label className="block scroll-mt-28">
              ...
              <input
                ...
                disabled={!canChangeName}
                className="mt-3 min-h-12 w-full rounded-full border border-white/[0.10] bg-black/[0.40] px-5 font-mono text-sm text-white outline-none placeholder:text-white/[0.24] disabled:opacity-45"
              />
            </label>
          </div>
          {!canChangeName ? <p className="mt-3 text-xs leading-6 text-velmere-gold/[0.80]">{t("cooldown.rule")}</p> : null}
          <label className="mt-5 block scroll-mt-28">
            ...
            <textarea
              ...
              className="mt-3 min-h-28 w-full resize-none rounded-[1.5rem] border border-white/[0.10] bg-black/[0.40] p-4 text-sm leading-7 text-white outline-none placeholder:text-white/[0.24]"
            />
          </label>
          <button
            type="button"
