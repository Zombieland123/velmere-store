...
const rawVisibleKeyPattern =
  />\s*[^<]*(navigation\.drawer|navigation\.locales|Home\.heroImageAlt|Home\.productTag|(?<![a-z])Vlm\.|(?<![a-z])Legal\.|(?<![a-z])Token\.)[^<]*</i;
...
const staleVisiblePattern =
  />\s*[^<]*(Riemann constraints|impenetrable|secured by Bajak|Phantom connection rejected|15,000 VLM|Social Media|Paryż|Warszawa|On-chain|Audio Wył)[^<]*</i;
